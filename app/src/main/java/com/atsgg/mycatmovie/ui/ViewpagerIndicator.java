...
public class ViewpagerIndicator extends LinearLayout {
    ...
    private void init() {
        this.setBackgroundResource(R.drawable.vpi_bg);
        paint = new Paint();
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (isSetData) {
            ...
            for (int i = 0; i < mDatas.size(); i++) {
                ...
                tv.setTextColor(getResources().getColor(R.color.home_back_selected));
        ...
        if (getChildCount() > 0) {
            for (int i = 0; i < getChildCount(); i++) {
                if (i == currentPosition) {
                    ((TextView) getChildAt(currentPosition)).setTextColor(UIUtils.getColor(R.color.home_back_selected));
                } else {
                    ((TextView) getChildAt(i)).setTextColor(UIUtils.getColor(R.color.white));
                }
            }
        }
    }
